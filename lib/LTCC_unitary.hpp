...
public:
	LTCC_unitary(double v) {
		const double pca = 11.9*1.5*0.5; // if CDI fca = 3;, then sc.ICaL_scale  = 0.5 // 14:45:31, Tue, 23-June-2020, By Haibo
		z = v * F / (R * T);
		za = z * 2.0;
	...
	double compute_LTCC_unitary(double Caj, double cao) {
		...
		if (fabs(za) < 0.1)
		{
			ica = (factor1 / (2.0 * F / (R * T))) * (Caj * 0.001 * exp(za) - cao); // [uM] *) xw: should be [uM/ms]
		}
		if (ica > 0) ica = 0;
		return ica;
	}
